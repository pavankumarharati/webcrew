...
const included = [
  'Custom website in your style of choice, built and deployed automatically — on your own domain, yours to keep',
  'Exact Google Maps location embedded — pulled straight from your business listing',
  ...
  'Call transcript + summary sent after every call',
  ...
  'Monthly traffic + ranking report, 97+ PageSpeed, SSL, mobile-perfect out of the box',
...
const styleOptions = [
  {
    id: 'editorial',
    name: 'Editorial',
    tagline: 'Bold, magazine-style, content-forward',
    body: 'Big condensed headlines, numbered service cards, alternating dark/light bands. Built for businesses that want to look established and premium.',
  },
  {
    id: 'zigzag',
    name: 'Professional',
    tagline: 'Clean, structured, classic local-business layout',
    body: 'Straightforward hero, feature grid, stats, reviews, service-area map. Built for businesses that want a polished, no-nonsense site.',
  },
] as const

const industries = ['Contractors', 'Plumbers', 'Electricians', 'HVAC', 'Roofers', 'Cleaning', 'Auto services', 'Salons', 'Spas', 'Home services']
const industryPages: Record<string, string> = { Contractors: '/contractors', Plumbers: '/plumbing', Electricians: '/electricians', HVAC: '/hvac', Roofers: '/roofing', Cleaning: '/cleaning', 'Auto services': '/auto-services', Salons: '/salons', Spas: '/spas', 'Home services': '/home-services' }

const faqs = [
  ['What is WebCrew?', 'WebCrew is an AI front office for local businesses. It answers every inbound call 24/7, qualifies leads, books appointments, sends you a summary after every call, and follows up automatically — plus a custom website (pick your style) built and deployed automatically. One flat monthly plan.'],
  ['Do I need to do anything besides fill out the form?', 'No. You pick a style, tell us about your business, and the build, deploy, and AI Reception setup all happen automatically — no calls, no back-and-forth, no manual steps on our side.'],
  ...
  ['How fast does WebCrew build a website?', 'Most clients wake up to a text with their live site link the morning after submitting the form — the AI pipeline builds and deploys a complete custom website in about 6 hours with no human involvement, averaging 97/100 on Google PageSpeed.'],
  ['What does WebCrew cost?', '$0 setup fee and a 30-day free trial — a card is required to start the trial, but nothing is charged until it ends. After that it is a flat $297/month for the website and AI receptionist, no add-ons or tiers.'],
  ['Can I see it work before committing?', 'Yes. Call the live AI Reception line at (918) 255-5151 to test it yourself, or request a walkthrough tailored to your business.'],
]

function Cta({ children = 'See what I’m losing', dark = false }: { children?: React.ReactNode; dark?: boolean }) {
  return (
    <a className={`wc2-cta${dark ? ' wc2-cta-dark' : ''}`} href="#lead-form">
...
export default function MissedCallLanding({ showPricing = false }: { showPricing?: boolean }) {
  ...
  const [lead, setLead] = useState({ name: '', business: '', email: '', phone: '', website: '', niche: '', style: 'editorial', city: '', state: '', mapsLink: '', pain: '', missedCalls: '', urgency: '', notes: '', smsConsent: false })
  ...
  useEffect(() => {
    ...
    if (niche && industries.includes(niche)) setLead(f => ({ ...f, niche }))
  }, [])
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? 'https://api.webcrew.app'}/leads`, {
        ...
        body: JSON.stringify({
          ...
          templateStyle: lead.style,
          city: lead.city,
          state: lead.state,
          googleMapsLink: lead.mapsLink || undefined,
          service: 'AI front office enquiry',
          message: `Pain point: ${lead.pain || 'Not provided'} | Missed calls/week: ${lead.missedCalls || 'Not provided'} | Urgency: ${lead.urgency || 'Not provided'} | Prospect notes: ${lead.notes || 'Not provided'}`,
  ...
  const links = [
    ['Styles', '#styles'], ['The problem', '#problem'], ['How it works', '#how-it-works'], ['What’s included', '#included'], ['Who it’s for', '#industries'], ['FAQ', '#faq'],
  ...
  return (
    <div className="wc2-page">
      ...
      <nav className="wc2-nav">
        <a href="#top" className="wc2-logo" aria-label="WebCrew home"><BrandMark size={32} className="wc2-logo-pop" />WebCrew</a>
      ...
      <main id="top">
        <section className="wc2-hero" data-avatar-caption="Hey — I'm WebCrew's AI. I'll show you around, or just ask me anything.">
          <div className="wc2-hero-copy">
            ...
            <small>$0 setup fee. 30-day free trial. See it working first.</small>
        ...
        <section className="wc2-section wc2-styles" id="styles" data-avatar-caption="Every plan includes a website in whichever style fits your business.">
          <Eyebrow>Included with every plan</Eyebrow>
          <div className="wc2-split-head"><h2>Your website, <em>your style.</em></h2><p>Every WebCrew plan includes a custom website — generated from real templates, not a generic AI slop layout. Pick the style that fits your business; the AI receptionist is what actually runs your front office.</p></div>
          <div className="wc2-style-grid">
            {styleOptions.map((s) => (
              <button
                type="button"
                key={s.id}
                className={`wc2-style-card${lead.style === s.id ? ' active' : ''}`}
                onClick={() => { setLead({ ...lead, style: s.id }); document.getElementById('lead-form')?.scrollIntoView({ behavior: 'smooth' }) }}
              >
                <span className="wc2-style-check">{lead.style === s.id ? <Check size={13} /> : null}</span>
                <h3>{s.name}</h3>
                <p className="wc2-style-tagline">{s.tagline}</p>
                <p>{s.body}</p>
              </button>
            ))}
          </div>
        ...
        <section className="wc2-section wc2-explainer" data-avatar-caption="Here's what an AI front office actually is, in plain terms.">
          <div className="wc2-explainer-inner">
            <h2>What is an AI front office?</h2>
            <p>An AI front office is a system that answers your business&rsquo;s calls, texts, and website inquiries the way a trained receptionist would — greeting the caller, understanding what they need, checking availability, and booking the appointment, without a human standing by around the clock. WebCrew builds one specifically for your business: it learns your services, hours, service area, and booking rules before it ever answers a call, so it sounds like your business instead of a generic voice assistant. Every call gets a transcript and summary sent to you afterward, every lead gets an instant SMS or email alert, and every missed conversation gets an automatic follow-up on day 3 and day 10. It doesn&rsquo;t replace your team — it makes sure no call, text, or website visitor is ever the one that got away because nobody was free to answer.</p>
          </div>
        </section>
        ...
        <section className="wc2-section wc2-included" id="included" data-avatar-caption="Here's the full list — nothing hidden, nothing upsold later.">
          <Eyebrow>What&apos;s included</Eyebrow>
          <div className="wc2-split-head"><h2>One plan. <em>Everything your front office needs.</em></h2><p>No add-ons to discover later. Every WebCrew plan includes all ten of these from day one.</p></div>
          <ul className="wc2-included-grid">{included.map(x => <li key={x}><CheckCircle2 /><span>{x}</span></li>)}</ul>
        </section>
        ...
        <section className="wc2-dark wc2-industries" id="industries" data-avatar-caption="Sound like your business? This was built for exactly that."><div className="wc2-dark-inner stacked"><Eyebrow light>Built for local businesses</Eyebrow><h2>Built for businesses that can’t stop working to <em>answer every call.</em></h2><div className="wc2-industry-grid">{industries.map((x, i) => industryPages[x] ? <a key={x} href={industryPages[x]}><span>{String(i + 1).padStart(2, '0')}</span>{x}</a> : <div key={x}><span>{String(i + 1).padStart(2, '0')}</span>{x}</div>)}</div><p>For local teams where <b>one missed call can mean one lost job.</b></p><Cta dark>See if WebCrew fits my business</Cta></div></section>
        ...
        <section className="wc2-final" id="contact" data-avatar-caption="Ready? Tell me about your business and I'll take it from here."><div className="wc2-final-glow" /><div className="wc2-final-grid"><div className="wc2-final-copy"><div className="wc2-pill dark"><Sparkles size={14} /> $0 setup fee · 30-day trial</div><h2>Tell us where calls are <em>slipping through.</em></h2><p>Share the pressure points in your business. WebCrew will use them to prepare the right call-answering and follow-up workflow for you.</p><div className="wc2-form-benefits"><span><CheckCircle2 /> $0 setup fee — everything included, one flat plan</span><span><CheckCircle2 /> 30-day free trial, full access — card required, no charge until trial ends</span><span><CheckCircle2 /> Personalized front-office walkthrough</span><span><CheckCircle2 /> Clear missed-revenue diagnosis</span><span><CheckCircle2 /> Human review before automation begins</span></div></div>
          ...
          <form className="wc2-lead-form" id="lead-form" onSubmit={submitLead}>
            ...
            <div className="wc2-field-grid"><label><span>City *</span><input required value={lead.city} onChange={e => setLead({ ...lead, city: e.target.value })} placeholder="Tracy" /></label><label><span>State *</span><input required maxLength={2} value={lead.state} onChange={e => setLead({ ...lead, state: e.target.value.toUpperCase() })} placeholder="CA" /></label></div>
            <label><span>Google Maps link <small>(optional — gives your site an exact map pin)</small></span><input type="url" value={lead.mapsLink} onChange={e => setLead({ ...lead, mapsLink: e.target.value })} placeholder="Paste your business's Google Maps share link" /></label>
            <div className="wc2-field-grid">
              <label><span>Business type *</span><select required value={lead.niche} onChange={e => setLead({ ...lead, niche: e.target.value })}><option value="">Select one</option>{industries.map(x => <option key={x}>{x}</option>)}<option>Other local business</option></select></label>
              <label><span>Website style *</span><select required value={lead.style} onChange={e => setLead({ ...lead, style: e.target.value })}>{styleOptions.map(s => <option key={s.id} value={s.id}>{s.name} — {s.tagline}</option>)}</select></label>
            </div>
            <label><span>Calls missed each week</span><select value={lead.missedCalls} onChange={e => setLead({ ...lead, missedCalls: e.target.value })}><option value="">Not sure</option><option>1–3</option><option>4–7</option><option>8–15</option><option>16+</option></select></label>
            <label><span>What is costing you the most right now? *</span><select required value={lead.pain} onChange={e => setLead({ ...lead, pain: e.target.value })}><option value="">Choose your biggest pain point</option><option>Missing calls while on jobs</option><option>Calls going unanswered after hours</option><option>Slow lead follow-up</option><option>Not qualifying callers before booking</option><option>Too much time spent answering repeat questions</option><option>Leads disappearing before they schedule</option><option>Don't have a website yet</option><option>Other</option></select></label>
            ...
            <label><span>{lead.niche === 'Other local business' ? 'What kind of business is it? *' : 'Tell us what is happening in your business'} {lead.pain === 'Other' || lead.niche === 'Other local business' || lead.urgency === 'Other' ? '*' : <small>(optional)</small>}</span><textarea required={lead.pain === 'Other' || lead.niche === 'Other local business' || lead.urgency === 'Other'} maxLength={1200} value={lead.notes} onChange={e => setLead({ ...lead, notes: e.target.value })} placeholder={lead.niche === 'Other local business' ? 'e.g. pet grooming, tutoring, pressure washing — any business works, just tell us what it is' : 'What are you losing time or customers on? What have you tried? What would a good result look like for you?'} /><small className="wc2-field-help">We use this to prepare useful recommendations before we call, text, or email you.</small></label>
            <label className="wc2-consent"><input type="checkbox" required checked={lead.smsConsent} onChange={e => setLead({ ...lead, smsConsent: e.target.checked })} /><span><strong>Yes, I agree to receive recurring SMS messages from WebCrew</strong> about my enquiry, requested demo, and related services. Message frequency varies. Message and data rates may apply. Consent is not a condition of purchase. Reply <strong>STOP</strong> to unsubscribe or <strong>HELP</strong> for help. <a href="/privacy" target="_blank" rel="noreferrer">Privacy Policy</a> &amp; <a href="/terms" target="_blank" rel="noreferrer">Terms</a>.</span></label>
            ...
            <small className="wc2-form-note">SMS consent is required to submit this enquiry, but is never a condition of any purchase.</small>
          </form>}
        </div></section>
      </main>

      <footer className="wc2-footer"><a href="#top" className="wc2-logo"><BrandMark size={32} />WebCrew</a><p>AI front office for local businesses.</p><div><a href="/privacy">Privacy</a><a href="/terms">Terms</a><a href="mailto:hello@webcrew.app">Contact</a></div></footer>

      <style>{`
        :root{--wc-navy:#0d172b;--wc-navy2:#172238;--wc-orange:${ORANGE};--wc-bg:#f6f7f9;--wc-ink:#10182b;--wc-muted:#667085;--wc-line:#e5e8ee}
        .wc2-page{visibility:visible;background:var(--wc-bg);color:var(--wc-ink);font-family:var(--font-body);overflow:hidden}.wc2-page h1,.wc2-page h2,.wc2-page h3,.wc2-page b,.wc2-page strong,.wc2-page .wc2-logo{font-family:var(--font-display)}
        ...
        .wc2-styles{background:#fff}.wc2-style-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:18px}.wc2-style-card{text-align:left;cursor:pointer;padding:30px 26px;border-radius:16px;border:2px solid var(--wc-line);background:var(--wc-bg);position:relative;transition:.2s}.wc2-style-card:hover{border-color:#ffcba3}.wc2-style-card.active{border-color:var(--wc-orange);background:#fff7f2;box-shadow:0 14px 34px rgba(255,107,26,.12)}.wc2-style-card h3{font-size:20px;margin:0 0 6px}.wc2-style-tagline{color:var(--wc-orange);font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px}.wc2-style-card>p:not(.wc2-style-tagline){color:var(--wc-muted);font-size:13px;line-height:1.65}.wc2-style-check{position:absolute;top:22px;right:22px;width:24px;height:24px;border-radius:50%;border:2px solid var(--wc-line);display:grid;place-items:center;color:#fff}.wc2-style-card.active .wc2-style-check{background:var(--wc-orange);border-color:var(--wc-orange)}
        .wc2-explainer{background:#fff;padding-top:0}.wc2-explainer-inner{max-width:760px;margin:auto;text-align:center}.wc2-explainer h2{font-size:clamp(26px,3.4vw,38px);letter-spacing:-.03em;margin-bottom:22px}.wc2-explainer p{font-size:15px;line-height:1.85;color:var(--wc-muted)}
        ...
        .wc2-included{background:#fff}.wc2-included-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:14px;list-style:none;padding:0;margin:0}.wc2-included-grid li{display:flex;align-items:flex-start;gap:12px;padding:18px 20px;border:1px solid var(--wc-line);border-radius:12px;background:var(--wc-bg)}.wc2-included-grid svg{width:18px;flex:0 0 18px;color:#12b76a;margin-top:2px}.wc2-included-grid span{font-size:13px;line-height:1.55;font-weight:600;color:var(--wc-ink)}
        ...
        .wc2-dark-inner.stacked{display:block;text-align:center}.wc2-dark-inner.stacked>.wc2-eyebrow{justify-content:center}.wc2-industries h2{max-width:800px;margin:0 auto 45px}.wc2-industry-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:10px}.wc2-industry-grid div,.wc2-industry-grid a{display:block;background:var(--wc-navy2);border:1px solid #28344c;border-radius:11px;padding:20px;text-align:left;font-family:var(--font-display);font-weight:700;font-size:13px;color:#fff;text-decoration:none;transition:border-color .2s}.wc2-industry-grid a:hover{border-color:var(--wc-orange)}.wc2-industry-grid span{display:block;color:var(--wc-orange);font-size:9px;margin-bottom:20px}.wc2-industries .stacked>p{color:#aab3c3;margin:30px 0 25px;font-size:13px}
        ...
        @keyframes wc2-logo-pop{0%{transform:scale(.25);box-shadow:0 0 0 0 rgba(255,107,26,0)}55%{transform:scale(1.1);box-shadow:0 0 18px 4px rgba(255,107,26,.4)}100%{transform:scale(1);box-shadow:0 0 0 0 rgba(255,107,26,0)}}.wc2-logo-pop{animation:wc2-logo-pop .7s cubic-bezier(.34,1.56,.64,1) both}
        @media(prefers-reduced-motion:reduce){.wc2-page *{scroll-behavior:auto!important;transition:none!important}.wc2-logo-pop{animation:none}}
      `}</style>
    </div>
  )
}
