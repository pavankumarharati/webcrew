...
export default function FeatureLanding({ f }: { f: FeatureContent }) {
  ...
  return (
    <div className="wcn-page">
      <nav className="wcn-nav">
        ...
        <a href="/" className="wcn-back">← Back to WebCrew</a>
      </nav>

      <main>
        <section className="wcn-hero">
          <div className="wcn-pill"><ShieldCheck size={13} /> {f.pill}</div>
          <h1>{f.headline}</h1>
          <p>{f.sub}</p>
          <div className="wcn-actions">
            <a className="wcn-cta" href={ctaHref}><span>See it live for my business</span><ArrowRight size={16} strokeWidth={2.5} /></a>
        ...
        <section className="wcn-triage">
          <div className="wcn-eyebrow"><span />{f.sectionEyebrow}</div>
        ...
        <section className="wcn-included">
          <h2>Included in the same flat plan — not a separate product.</h2>
          <ul>
            {f.included.map(i => (
              <li key={i}><CheckCircle2 /><span>{i}</span></li>
            ))}
          </ul>
        </section>
